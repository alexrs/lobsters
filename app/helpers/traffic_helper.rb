...
module TrafficHelper
  ...
  def self.traffic_range
    ...
    #     from
    #       (SELECT count(1) AS n_votes,
    #         floor(UNIX_TIMESTAMP(updated_at)/#{div})
    #         AS period FROM votes    WHERE updated_at >= '#{start_at}' GROUP BY period) v,
    #       (SELECT count(1) AS n_comments,
    # floor(UNIX_TIMESTAMP(created_at)/#{div})
    # AS period FROM comments WHERE created_at >= '#{start_at}' GROUP BY period) c,
    #       (SELECT count(1) AS n_stories,
    # floor(UNIX_TIMESTAMP(created_at)/#{div})
    # AS period FROM stories  WHERE created_at >= '#{start_at}' GROUP BY period) s
    #     where
